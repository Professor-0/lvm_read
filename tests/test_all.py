...
def test_short_lvm():
    ...
    np.testing.assert_equal(data['segments'][0]['data'][0][1][0], 0.914018)
...
def test_with_empty_fields_lvm():
    ...
    np.testing.assert_equal(data['segments'][0]['data'][6][1][0], -0.011923)


def test_with_multi_time_column_lvm():
    data = read('./data/multi_time_column.lvm',
                read_from_pickle=False, dump_file=False)
    np.testing.assert_equal(data['segments'][0]['data'][0][0][0], 0.000000)
    np.testing.assert_equal(data['segments'][0]['data'][0][1][0], -0.035229)
    np.testing.assert_equal(data['segments'][0]['data'][1][0][0], 0.000000)
    np.testing.assert_equal(data['segments'][0]['data'][1][1][0], 0.532608)


def test_no_decimal_separator():
    data = read('./data/no_decimal_separator.lvm',
                read_from_pickle=False, dump_file=False)
    np.testing.assert_equal(data['segments'][0]['data'][0][1][0], -0.008807)
